...
import models.*
import services.consumers.{AchievableBatch, MergeableBatch, StagedVersionedBatch}
...
class StagingProcessorTests extends AsyncFlatSpec with Matchers with EasyMockSugar:
  ...
  it should "write data rows grouped by schema to staging tables" in  {
    ...
    def toInFlightBatch(batches: Iterable[StagedVersionedBatch & MergeableBatch & AchievableBatch], index: Long, others: Any): stagingProcessor.OutgoingElement =
  ...
  it should "allow accessing stream metadata" in {
    ...
    class IndexedStagedBatchesWithMetadata(override val groupedBySchema: Iterable[StagedVersionedBatch & MergeableBatch & AchievableBatch],
    ...
    def toInFlightBatch(batches: Iterable[StagedVersionedBatch & MergeableBatch & AchievableBatch], index: Long, others: Chunk[Any]): stagingProcessor.OutgoingElement =
