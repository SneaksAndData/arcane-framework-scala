...
import models.*
import services.consumers.{ArchivableeBatch, MergeableBatch, StagedVersionedBatch}
...
class StagingProcessorTests extends AsyncFlatSpec with Matchers with EasyMockSugar:
  ...
  it should "write data rows grouped by schema to staging tables" in  {
    ...
    def toInFlightBatch(batches: Iterable[StagedVersionedBatch & MergeableBatch & ArchivableeBatch], index: Long, others: Any): stagingProcessor.OutgoingElement =
  ...
  it should "allow accessing stream metadata" in {
    ...
    class IndexedStagedBatchesWithMetadata(override val groupedBySchema: Iterable[StagedVersionedBatch & MergeableBatch & ArchivableeBatch],
    ...
    def toInFlightBatch(batches: Iterable[StagedVersionedBatch & MergeableBatch & ArchivableeBatch], index: Long, others: Chunk[Any]): stagingProcessor.OutgoingElement =
