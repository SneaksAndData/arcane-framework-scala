...
class UpsertBlobTests extends AnyFlatSpec with Matchers:
  ...
  it should "generate a valid merge with partitions" in {
    ...
    val expected =
      Using(Source.fromURL(getClass.getResource("/generate_a_valid_merge_query_with_partitions_upsertblob.sql"))) {
        _.getLines().mkString("\n")
      }.get
  ...
  "UpsertBlobMergeBatch" should "generate a valid versioned batch" in {
    val batch = UpsertBlobMergeBatch(
      "test.staged_a",
      Seq(
        MergeKeyField,
        Field(
    ...
    val expected =
      Using(Source.fromURL(getClass.getResource("/generate_a_valid_upsertblob_merge_query_with_partitions.sql"))) {
        _.getLines().mkString("\n")
      }.get
  ...
  private val mergeKeyStatements = Table(
    ...
    (Seq("bucket(ARCANE_MERGE_KEY, 32)"), "filter_out_single_arcane_merge_key_from_merge_match_upsertblob"),
    (
      Seq("bucket(ARCANE_MERGE_KEY, 32)", "bucket(colA, 32)", "year(colB)"),
      "filter_out_arcane_merge_key_from_merge_match_upsertblob"
    )
  )

  "UpsertBlobMergeBatch" should "filter out arcane merge key from merge match" in {
    val batchSchema = Seq(
      MergeKeyField,
      Field(
        name = "colA",
