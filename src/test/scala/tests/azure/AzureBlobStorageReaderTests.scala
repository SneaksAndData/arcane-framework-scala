...
object AzureBlobStorageReaderTests extends ZIOSpecDefault:
  override def spec: Spec[TestEnvironment & Scope, Any] = suite("AzureBlobStorageReader")(
    Map(
      ...
      ("MissingFolder/", false),
      ("MissingFolder", false)
    ).map { case (blobName, expectedResult) =>
      test(s"correctly check if a blob $blobName exists in a container") {
        for
      ...
      test(s"stream blob $blobName content from a container") {
        for
          path <- ZIO.succeed(AdlsStoragePath(s"abfss://$container@$storageAccount.dfs.core.windows.net/$blobName").get)
          content <- ZIO
            .acquireReleaseWith(storageReader.streamBlobContent(path))(reader => ZIO.succeed(reader.close())) {
              reader =>
