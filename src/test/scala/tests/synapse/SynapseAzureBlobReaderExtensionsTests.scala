...
object SynapseAzureBlobReaderExtensionsTests extends ZIOSpecDefault:
  /** Test cases for getRootPrefixes method. These test cases relying on the following assumptions:
    ...
    *   - The test container was cleaned up before running the populate-cdm-container.py script Changing this behavior
    *     is too expensive for the current scope. This will be fixed in
    *     https://github.com/SneaksAndData/arcane-framework-scala/issues/44 If tests are failing, please ensure that
    *     cdm-e2e container is empty and run populate-cdm-container.py script
    */
  private def rootPrefixesTests: immutable.Iterable[Spec[Any, Throwable]] =
    ...
    testCases.map { case (startDate, expectedCount) =>
      test("reads root prefixes correctly") {
        for
          ...
          prefixes <- storageReader
            .getEligibleDates(path, startDate, OffsetDateTime.now(ZoneOffset.UTC).plusHours(12))
