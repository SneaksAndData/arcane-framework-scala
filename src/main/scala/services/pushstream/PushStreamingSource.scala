...
class PushStreamingSource(
  ...
  override def getShards(rangeStart: WatermarkType, rangeEnd: WatermarkType): ZStream[Any, Throwable, ShardMetadata] = ZStream.empty
  ...
  private def runDynamoQuery(queryRequest: QueryRequest): Task[QueryResponse] = for
    result <- ZIO.attemptBlocking(dynamodbClient.query(queryRequest))
  yield result
  
  private def getAvroSchema: Task[AvroSchema] = this.sinkPropertyManager.getTableSchema(sourceTableName)
