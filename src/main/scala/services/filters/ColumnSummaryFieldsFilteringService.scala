 ...
 *
 * @param fieldSelectionRule The field selection rule to use.
 */
class ColumnSummaryFieldsFilteringService(fieldSelectionRule: FieldSelectionRuleSettings) extends MsSqlServerFieldsFilteringService:
  ...
  def filter(fields: List[ColumnSummary]): Try[List[ColumnSummary]] = fieldSelectionRule.rule match
    case includeFields: FieldSelectionRule.IncludeFields =>
      ...
      excludedPks match
        case Nil =>
          Success(fields.filter(entry => includeFields.fields.exists(f => entry._1.normalizeName.toLowerCase().equalsIgnoreCase(f))))
        case _ =>
          Failure(new IllegalArgumentException(s"Fields ${toString(excludedPks)} are primary keys, and must be included in the field selection rule"))
    ...
    case excludeFields: FieldSelectionRule.ExcludeFields =>
      ...
      excludedPks match
        case Nil =>
          Success(fields.filter(entry => !excludeFields.fields.exists(f => entry._1.normalizeName.toLowerCase().equalsIgnoreCase(f))))
        case _ =>
          Failure(new IllegalArgumentException(s"Fields ${toString(excludedPks)} are primary keys, and cannot be filtered out by the field selection rule"))
...
object ColumnSummaryFieldsFilteringService:
   ...
   *
   * @param fieldSelectionRule The field selection rule to use.
   * @return A new ColumnSummaryFieldsFilteringService.
   */
