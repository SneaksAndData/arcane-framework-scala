...
final case class S3StoragePath(bucket: String, objectKey: String) extends BlobPath:
  ...
  /** Joins the given key name to the current path.
    ...
    */
  @targetName("plus")
  override def +(keyName: String): this.type =
    copy(objectKey = if (objectKey.isEmpty) keyName else s"$objectKey/$keyName").asInstanceOf[this.type]
...
object S3StoragePath:
  ...
  def applySafe(hdfsPath: String): Try[S3StoragePath] =
    if hdfsPath.startsWith(protocolName) then apply(hdfsPath) else apply(s"$protocolName://$hdfsPath")
