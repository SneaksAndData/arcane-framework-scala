...
object IcebergCatalogCredential extends IcebergCatalogCredential:
  override val credential: String = sys.env.getOrElse("ARCANE_FRAMEWORK__S3_CATALOG_AUTH_CLIENT_ID", "") + ":" + sys.env
    .getOrElse("ARCANE_FRAMEWORK__S3_CATALOG_AUTH_CLIENT_SECRET", "")
  override val oauth2Uri: String = sys.env.get("ARCANE_FRAMEWORK__S3_CATALOG_AUTH_CLIENT_URI") match
    case Some(uri) => uri
    case None =>
      throw InstantiationError("ARCANE_FRAMEWORK__S3_CATALOG_AUTH_CLIENT_URI environment variable is not set")
  ...
  override val oauth2TokenRefreshEnabled: Boolean =
    sys.env.getOrElse("ARCANE_FRAMEWORK__S3_CATALOG_AUTH_TOKEN_REFRESH_ENABLED", "false").toLowerCase == "true"
  override val oauth2SessionTimeoutMs: Long = sys.env
    .getOrElse("ARCANE_FRAMEWORK__S3_CATALOG_AUTH_SESSION_TIMEOUT_MILLIS", Duration.ofMinutes(55).toMillis.toString)
    .toLong

  final val oAuth2Properties: Map[String, String] =
    val authProperties =
      if oauth2StaticToken != "" then
        Map(
