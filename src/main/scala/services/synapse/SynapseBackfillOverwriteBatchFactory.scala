 ...
 */
class SynapseBackfillOverwriteBatchFactory(jdbcMergeServiceClient: JdbcMergeServiceClient,
...
object SynapseBackfillOverwriteBatchFactory:
   ...
   * @param targetTableSettings     The target table settings.
   * @param tablePropertiesSettings The table properties settings.
   * @return The SynapseBackfillOverwriteBatchFactory instance.
   */
  def apply(jdbcMergeServiceClient: JdbcMergeServiceClient,
            backfillSettings: BackfillSettings,
            targetTableSettings: TargetTableSettings,
            tablePropertiesSettings: TablePropertiesSettings): SynapseBackfillOverwriteBatchFactory =
    new SynapseBackfillOverwriteBatchFactory(jdbcMergeServiceClient, backfillSettings, targetTableSettings, tablePropertiesSettings)
  ...
  val layer: ZLayer[Environment, Nothing, BackfillOverwriteBatchFactory] =
    ZLayer {
      ...
      yield SynapseBackfillOverwriteBatchFactory(mergeServiceClient, backfillSettings, targetTableSettings, tablePropertiesSettings)
    }
