...
final class SynapseBackfillSourceDataProvider(
  ...
  (
    dataProvider, backfillSettings, stagingTableSettings, sinkSettings, metricTagProvider
  ):

  override protected def backfillStream(backfillStart: SynapseWatermark, backfillEnd: SynapseWatermark): ZStream[Any, Throwable, BootstrappedShard] =
    dataProvider.getData(backfillStart, backfillEnd)
      .map { stream => DefaultBootstrappedShard(
          shardStream = stream,
