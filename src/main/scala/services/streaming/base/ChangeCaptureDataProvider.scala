...
trait ChangeCaptureDataProvider[DataVersionType <: SourceWatermark[String]]:
  ...
  /** Find latest watermark in between provided ones, taking that no more than `maxRangeSize` watermarks can be taken into evaluation
   * For example, if diff(startWatermark, endWatermark) contains 10 elements [wm0, wm1, wm2, ...], but maxRangeSize is set to 3, the returned watermark will be `wm2`
    */
  def getLatestWatermarkInRange(startWatermark: DataVersionType, endWatermark: DataVersionType, maxRangeSize: Int): Task[DataVersionType]
