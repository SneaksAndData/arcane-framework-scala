...
object WatermarkProcessingExtensions:
  extension (batch: StagedBatch) def applyWatermark(writer: IcebergS3CatalogWriter, targetName: String): ZIO[Any, Throwable, Unit] = 
    for
      _ <- ZIO.when(batch.completedWatermarkValue.isDefined) {
        for
          watermark <- ZIO.attempt(batch.completedWatermarkValue.get)
          _ <- zlog("Batch %s completed stream from watermark %s, will update target watermark", batch.name, watermark)
