...
class MemoryBoundShaper(tablePropertyManager: TablePropertyManager, sinkSettings: SinkSettings) extends ThroughputShaper:
  ...
  private def estimateRowSize(schema: Schema): Long = schema.columns().asScala.map(_.`type`()).foldLeft(0L) {
    case (agg, tp) => tp.typeId() match
  ...
  override def estimateChunkSize: Task[(Elements: Int, ElementSize: Long)] = for
    ...
    _ <- ZIO.when(estimationCache.isEmpty) {
      for
        ...
        rowsSize <- ZIO.succeed(Seq(estimateRowSize(tableSchema).toDouble, tableSizeEstimate.Records / tableSizeEstimate.Size.toDouble).max)
        ...
        _ <- zlog("Computed baseline estimation parameters: memory cutoff %s and row size %s", memoryCutoff.toString, rowsSize.toString)
      yield ()  
    }
    
    
    chunkSizeFromRowSize <- ZIO.succeed(runtime.freeMemory() * estimationCache(memCacheKey) / estimationCache(rowSizeCacheKey))
    _ <- zlog("Estimated chunk size %s for the current stream", chunkSizeFromRowSize.toString)
    // TODO: add these as metrics
  yield if estimationCache("partitions").toInt > 1 then
    (Seq(chunkSizeFromRowSize, estimationCache("partitions") / 2).min.toInt, estimationCache(rowSizeCacheKey).toLong) 
  else
    (Seq(chunkSizeFromRowSize, 1.0).max.toInt, estimationCache(rowSizeCacheKey).toLong) // TODO: set baseline rate through settings

  override def estimateShapeBurst(chunkSize: Int, chunkElementSize: Long): Task[Int] = for
    chunksToFit <- ZIO.attempt(runtime.maxMemory() / (chunkSize * chunkElementSize))
  yield Seq(chunksToFit.toDouble / 2, 1.0).max.toInt  // TODO: set baseline burst and division factor through settings

  override def estimateShapeRate(chunkSize: Int, chunkElementSize: Long): Task[(Elements: Int, Period: Duration)] = for
    chunksToFit <- ZIO.attempt(runtime.maxMemory() / (chunkSize * chunkElementSize))
