...
sealed trait MergeQuerySegment:
  ...
  override def toString: String =
    Seq(segmentPrefix, segmentCondition.map(c => s"AND $c").getOrElse(""), "THEN", segmentAction).mkString(" ")
...
trait WhenNotMatchedInsert extends MergeQuerySegment:
  ...
  override final def toString: String =
    ...
    Seq(
      segmentPrefix,
      segmentCondition.map(c => s"AND $c").getOrElse(""),
      "THEN",
      segmentAction,
      s"($columnList)",
      "VALUES",
...
object OnSegment:
  private def generateInClause(content: String, partName: String): String =
    s"${MergeQueryCommons.TARGET_ALIAS}.$partName IN ($content)"

  def apply(partitionValues: Map[String, List[String]], mergeKey: String, extraMatchKeys: Seq[String]): OnSegment =
    new OnSegment {
      override val segmentCondition: Option[String] =
        ...
        val inClause = partitionValues
          .map(values => generateInClause(values._2.map(part => s"'$part'").mkString(","), values._1))
          .mkString(" AND ")
        val extraMatchClause = extraMatchKeys
          .map(key =>
            s"(${MergeQueryCommons.TARGET_ALIAS}.$key = ${MergeQueryCommons.SOURCE_ALIAS}.$key OR ${MergeQueryCommons.SOURCE_ALIAS}.$key IS NULL)"
          )
          .mkString(" AND ")
