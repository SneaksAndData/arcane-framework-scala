...
object SynapseLinkShardCommitQuery:
  def apply(targetName: String, combineTableName: String): OverwriteQuery = OverwriteReplaceQuery(
    sourceQuery = s"""SELECT * FROM (
                     | SELECT * FROM $combineTableName WHERE COALESCE(IsDelete, false) = false ORDER BY ROW_NUMBER() OVER (PARTITION BY ${MergeKeyField.name} ORDER BY versionnumber DESC) FETCH FIRST 1 ROWS WITH TIES
                     |)""".stripMargin,
