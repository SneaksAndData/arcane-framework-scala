...
/** Represents an SQL query used to update data in the target table
  * @param baseQuery
  *   SQL query skeleton to use when constructing the final query
  * @param segments
  *   Additional MERGE query segments
  */
case class MergeQuery(baseQuery: String, segments: Seq[MergeQuerySegment]) extends StreamingBatchQuery:
  ...
  def query: String =
    require(
      ...
      "OnSegment is not defined for this query, unable to generate runnable SQL"
    )
    require(
      segments.exists(seg =>
        seg match
...
object MergeQuery:
  ...
  def apply(targetName: String, sourceQuery: String): MergeQuery =
    new MergeQuery(baseQuery(targetName, sourceQuery), Seq())
