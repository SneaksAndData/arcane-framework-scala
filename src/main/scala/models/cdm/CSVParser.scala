...
object CSVParser:
  def parseCsvLine(line: String, delimiter: Char = ',', headerCount: Int): Seq[Option[String]] = {
    ...
    def extractValue(fromIndex: Int, currentPosition: Int): Option[String] =
      if currentPosition == fromIndex then
        if isDelimiter(fromIndex) then None
        ...
        (isDelimiter(currentPosition), isEol(currentPosition)) match
          ...
          case (true, true) if isQuote(currentPosition - 1) =>
            Some(
              line.slice(from = fromIndex, until = currentPosition - 1)
            ) // if there is a quote at the end, move back by 1 character
          case (true, true) => Some(line.slice(from = fromIndex, until = currentPosition))
          case (false, true) if !isQuote(currentPosition) =>
            Some(line.slice(from = fromIndex, until = currentPosition + 1))
          case (false, true) if isQuote(currentPosition - 1) =>
            Some(line.slice(from = fromIndex, until = currentPosition - 1))
          case (true, false) if isQuote(currentPosition - 1) =>
            Some(line.slice(from = fromIndex, until = currentPosition - 1))
          case _ => Some(line.slice(from = fromIndex, until = currentPosition))

    val parsed = line.zipWithIndex
      .foldLeft((IndexedSeq[Option[String]](), 0, 0)) { (agg, element) =>
        val (character, charIndex)            = element
        val (result, quoteSum, prevCharIndex) = agg

        def handleEol: (IndexedSeq[Option[String]], Int, Int) =
          if isQuote(prevCharIndex) then (result :+ extractValue(prevCharIndex + 1, charIndex), quoteSum, prevCharIndex)
          else (result :+ extractValue(prevCharIndex, charIndex), quoteSum, prevCharIndex)
        ...
        character match
          ...
          case _ if (quoteSum == 0) && isDelimiter(charIndex) && !isEol(charIndex) =>
            ...
            else (result :+ extractValue(prevCharIndex, charIndex), quoteSum, charIndex + 1)

          case _ if (quoteSum == 0) && isDelimiter(charIndex) && isEol(charIndex) =>
            if isQuote(prevCharIndex) then
              (result :+ extractValue(prevCharIndex + 1, charIndex) :+ None, quoteSum, prevCharIndex)
            else (result :+ extractValue(prevCharIndex, charIndex) :+ None, quoteSum, prevCharIndex)
    ...
    if parsed.size != headerCount then
      throw new IllegalStateException(
        s"CSV line $line with delimiter $delimiter cannot be parsed into desired $headerCount fields"
      )
  ...
  def parseCSVLineToRow(csvLine: String, schema: ArcaneSchema): DataRow =
