...
case class CompletionShard(watermark: String,
                           override val shardTableName: String,                          
                           override val targetTableName: String, override val shardSourceEntityName: String, override val combinedTableName: String)
  extends StagedShard:
...
object CompletionShard:
  extension (shard: CompletionShard)
    def toCompleted: CompletedShard = CompletedShard(shard.shardId, shard.combinedTableName, shard.targetTableName, shard.shardSourceEntityName)