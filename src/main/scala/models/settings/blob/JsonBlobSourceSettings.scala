...
trait JsonBlobSourceSettings extends BlobSourceSettings:
  ...
  /**
   * Json source can automatically explode array fields into additional rows.
   * Out map key should contain a json pointer string to the json array field. Inner map links array property names with field names in Avro Schema.
   * Given fields in Avro Schema: colA, colB, colC and source json: { "rootColA": "abc", "lines": [{ "a":1, "b":2, "c": 3}, { "a":1, "b":2, "c": 3}] }
   * the value for this setting should be: Map("/lines" -> Map("a" -> "colA", "b" -> "colB", "c" -> "colC"))
   * 
   * If not provided, nested and array fields will be recorded as Iceberg Variant type (ObjectType in Arcane) 
   * and target table will require readers to support Iceberg V3 or later.
   */
